...
namespace DAL
{
    public class SerieRepository
    {
        ...
        public List<Aflevering> GetAfleveringen(AfleveringSortEnum toegevoegd, int top)
        {
            ...
            {
                default:
                    return ctx.Afleveringen.OrderByDescending(a => a.Toegevoegd).ThenByDescending(s => s.SerieID).ThenByDescending(a => a.Seizoen).ThenByDescending(a => a.Nummer).Take(top).ToList();
            }
        }
